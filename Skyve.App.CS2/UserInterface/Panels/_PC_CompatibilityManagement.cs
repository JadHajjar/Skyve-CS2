...
public partial class _PC_CompatibilityManagement : PanelContent
{
	...
	private _PC_CompatibilityManagement(bool load) : base(load)
	{
	...
	public _PC_CompatibilityManagement() : this(true)
	{
		PB_Loading.BringToFront();
	}

	public _PC_CompatibilityManagement(IEnumerable<ulong> packages) : this(false)
	{
	...
	public _PC_CompatibilityManagement(ReviewRequest request) : this(new[] { request.PackageId })
	{
		_request = request;
	}
	...
	private async void SetPackage(int page)
	{
		...
		try
		{
			...
			var catalogue = await skyveApiUtil.Catalogue(CurrentPackage!.Id);

			postPackage = catalogue?.Packages.FirstOrDefault()?.CloneTo<CompatibilityPackageData, PostPackage>();

			if (postPackage is null)
			{
				postPackage = (skyveDataManager).GetAutomatedReport(CurrentPackage).CloneTo<CompatibilityPackageData, PostPackage>();
			}
			else
			{
				var automatedPackage = (skyveDataManager).GetAutomatedReport(CurrentPackage).CloneTo<CompatibilityPackageData, PostPackage>();
	...
	private async Task<bool> Apply()
	{
		...
		postPackage.AuthorId = CurrentPackage.GetWorkshopInfo()?.Author?.Id?.ToString();
